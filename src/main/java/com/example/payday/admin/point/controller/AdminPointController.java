...
public class AdminPointController {
    ...
    @Operation(summary = "특정 유저의 포인트 요약 및 내역 조회", description = "관리자 권한으로 특정 회원의 포인트 충전/환불 요약 정보와 함께 내역 리스트를 페이지네이션으로 조회합니다.")
    @ApiResponses({
    ...
    @Operation(summary = "전체 포인트 내역 조회", description = "관리자 권한으로 전체 회원의 포인트 내역을 페이지네이션으로 조회합니다.")
    @ApiResponse(responseCode = "200", description = "정상 응답")
    ...
    @Operation(summary = "월간 총 충전/환불/순이익 통계 조회", description = "월간 기준으로 전체 포인트 충전액, 환불액, 순이익을 관리자 권한으로 조회합니다.")
    @ApiResponse(responseCode = "200", description = "정상 응답")
